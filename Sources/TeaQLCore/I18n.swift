...
public struct CheckResult:Sendable{public let ruleID:String;public let location:String;public let inputValue:String?;public let systemValue:String?;public var message:String?;public init(ruleID:String,location:String,inputValue:String?=nil,systemValue:String?=nil,message:String?=nil){self.ruleID=ruleID;self.location=location;self.inputValue=inputValue;self.systemValue=systemValue;self.message=message}}
...
public struct I18nCatalog:Sendable{
  ...
  public func translate(_ source:CheckResult,locale:TeaQLLocale)->CheckResult{var result=source;let keys=["required":"checker.required","min":"checker.min","max":"checker.max","min_str_len":"checker.minLength","min_length":"checker.minLength","max_str_len":"checker.maxLength","max_length":"checker.maxLength"];let key=keys[result.ruleID.lowercased()] ?? "checker.\(result.ruleID.lowercased())";result.message=message(locale,key).replacingOccurrences(of:"{location}",with:result.location).replacingOccurrences(of:"{system}",with:result.systemValue ?? "nil").replacingOccurrences(of:"{input}",with:result.inputValue ?? "nil").replacingOccurrences(of:"{input_len}",with:String(result.inputValue?.count ?? 0));return result}
}
